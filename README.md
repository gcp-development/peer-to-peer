...
The Multicast DNS (mDNS) protocol can be used in local networks (containers(https://kubernetes.io/docs/concepts/containers/) within the same [Pod](https://kubernetes.io/docs/concepts/workloads/pods/)). Multicast DNS (mDNS) does not process hostnames with other top-level domains (TLDs). Meaning we will need a implementation like [Kademlia](https://docs.ipfs.tech/concepts/dht/#kademlia) that is a data structure stored on multiple computers, scalable and fault-tolerant.
...
<ul>
  ...
  <li><a href="https://github.com/gcp-development/peer-to-peer/tree/main/kubernetes-setup" target="_self">p2p-setup</a> Kubernetes setup.</li>
</ul>
